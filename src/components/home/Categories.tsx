...
export default function Categories({ categories }: CategoriesProps) {
  ...
  return (
    <div id="categories" className="pt-8 mt-5 w-full bg-dark-600 rounded-xl overflow-hidden">
      <h4 className="text-xl md:text-2xl font-semibold mb-2 pl-5 text-white">Categories</h4>
      <table className="w-full">
        ...
        <tbody>
          {displayCategories.slice(0, 8).map((category) => (
            <tr key={category.id} className="border-b border-dark-500 hover:bg-dark-500/50 cursor-pointer">
              <td className="py-4 pl-5 font-bold text-gray-300">{category.name}</td>
              <td className="py-4 text-right font-medium text-gray-400">{category.coin_count}</td>
